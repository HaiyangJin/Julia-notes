...
# ╔═╡ 2687d3cc-1130-11ec-2a1b-0940f278dfdf
begin
	using DataFrames
	...
	TableOfContents(title="Effects of Interest")
end

# ╔═╡ 48aae3e2-2064-48ec-b790-c6f37ca50bb8
md"""
This note includes how to calculate effects of interest based on bootstrap results of the fitted model. 
"""

# ╔═╡ 34c98523-f8d8-4657-a13e-dcf666f76cae
md"# Fit the model"

# ╔═╡ 04db7027-eeea-4785-8e95-00119a11a5e0
df = DataFrame(Arrow.Table("data/example_sdif.arrow"))

# ╔═╡ 7fe8b315-ec6f-4cd0-996c-67d39b319814
lmm_rt = fit(
	MixedModel,
	@formula(RT ~ Condition * Congruency * Alignment + (1|Participant)),
	@subset(df, :isCorrect == 1),
	contrasts = merge(
		Dict(:Participant => Grouping()),
		Dict(:Condition => SeqDiffCoding(base="monocular")),
		Dict(:Congruency => SeqDiffCoding(base="congruent")),
		Dict(:Alignment => SeqDiffCoding(base="aligned")))
)

# ╔═╡ 61644f9d-6734-40af-b8ed-8c2ba87ad04f
issingular(lmm_rt)

# ╔═╡ b553dc02-ee39-4053-a3b9-d3ca86e33700
md"# Bootstrapping"

# ╔═╡ 2cb880dc-f113-418f-9970-7b01b8155989
bs_rt = parametricbootstrap(MersenneTwister(42), 1000, lmm_rt);

# ╔═╡ 3883f4df-5e65-4490-b588-10e325a241d4
beta_sample = DataFrame(bs_rt.β)

# ╔═╡ 2fce3cfa-f144-4f78-9037-6cd8f4fd34ce
# The figure is too larget to display
# ridgeplot(bs_rt)

# ╔═╡ cf5d73d9-fe28-491f-85c6-9c349b24c7c1
md"# Effects of interest"

# ╔═╡ 1c06f5c9-0748-4e08-ba91-1ec891bf9c71
md"Calculate the effects of interests based on the bootstrapping values:"

# ╔═╡ 51441edb-0ee1-4743-ba21-7d01961a1235
# Set(DataFrame(bs_rt.β).coefname)

# ╔═╡ c169300b-96d6-4811-af78-9ac8b5148e85
interest_sample = @chain DataFrame(bs_rt.β) begin
	@transform(:coefname = string.(:coefname))
	@transform(:coefname = 
		:coefname == "(Intercept)" ? "intercept" :
		:coefname == "Condition: CFS" ? "cond_main" :
		:coefname == "Congruency: incongruent" ? "cong_main" :
		:coefname == "Alignment: misaligned" ? "ali_main" :
		:coefname == "Condition: CFS & Congruency: incongruent" ? "cond_cong" :
		:coefname == "Condition: CFS & Alignment: misaligned" ? "cond_ali" :
		:coefname == "Congruency: incongruent & Alignment: misaligned" ? "cong_ali" :
		:coefname == "Condition: CFS & Congruency: incongruent & Alignment: misaligned" ? "cond_cong_ali" :
  		"other")
    unstack(:coefname, :β)
	@transform(
...
# ╔═╡ 98e0acc7-bfb5-442d-b751-43a729514e43
interest_sample

# ╔═╡ 6ad9dfe9-938a-4f53-a2c3-0b2bd723bdc1
md"## Shortest Interval"

# ╔═╡ 8043911b-8569-442c-ae75-303c96c918dd
@chain interest_sample begin
	@groupby(:variable)
	@combine(shortestcovint(:value))
end

# ╔═╡ d75de41e-2491-4fd5-8fba-be9ee8f05ebe
md"## Ridgeplot (to do)"

# ╔═╡ a9a6525d-c72e-4630-94a8-129ac4cbc3e7
# To do: plot the ridge plot for custom samples
# ridgeplot(interest_sample.value)
...
git-tree-sha1 = "83a256d773535a25851c820f71d2ffd91f52cace"
